<script>
  ...
  fullPageInfo.subscribe((val) => {
    ...
    animateCont = fpDestination === 'up' ? 'animate__slideInDown animate__delay-1s' : (fpDestination === 'down' ? 'animate__fadeOutUpBig animate__slower' : '')
    animateCont2 = fpDestination === 'up' ? 'animate__slideInDown animate__delay-1s' : (fpDestination === 'down' ? 'animate__fadeOutUp animate__slower animate__delay-1s' : '')
  })
</script>
    ...
    <MediaQuery query="(max-width: 992px)" let:matches>
      ...
      <div class="right {matches ? '' : 'animate__animated'} {animateCont2}" style="--animate-duration: 1000ms; --animate-delay: 200ms">
        <div class="computer-screen">
          <img src="./computer.svg" class="computer-graphic" alt="computer screen with coffee and objects" />
        </div>
      </div>
    </MediaQuery>
  </div>
  <img class="side-dots" src="./side-dots.png" alt="side-dots" aria-hidden />
...
<style>
  ...
  @media (min-width: 990px) {
    .left, .right {
      width: 45%; padding: 2vw; padding-right: 100px; padding-left: 0;
    }
  }
